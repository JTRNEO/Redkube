...
class resnet_mx_101_e2e_mask_satellite2(Symbol):
    ...
    def get_mask_head(self, feat, num_layers=4, num_classes=12):
        mask_relu_out = feat
        ...
        # Upsample the output
        # TODO: check the code here carefully
    ...
    def get_symbol_rcnn(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            # Put the Mask head
            mask_pred = self.get_mask_head(mask_deformable_roi_pool, num_classes=num_classes-1)

            # Compute the mask targets
            # TODO: check num_classes, ajust the default setting
            # TODO: check the hard code, choose right param of num_proposals
            # print 'before debug op'
            # pdb.set_trace()
            # mask_rois, gt_masks, mask_ids, mask_pred = mx.symbol.Custom(rois=mask_rois, mask_polys=gt_masks, mask_ids=mask_ids,
            #                  mask_pred=mask_pred, op_type='debug')
            ...
            # TODO: watch the code here: this is 2-way Softmax for N classes, so there are (num_classes-1) * 2
            #  channels from mask_pred
            ...
            cls_score = mx.sym.FullyConnected(name='cls_score', data=fc_new_2_relu, num_hidden=num_classes)
            bbox_pred = mx.sym.FullyConnected(name='bbox_pred', data=fc_new_2_relu, num_hidden=num_reg_classes * 4)
            # cls_score = mx.sym.Custom(data=cls_score, op_type='debug2', name='debug2')
            ...
            # seleft from one of 18 binary softmax channels
