...
from cpu_nms import cpu_nms, cpu_soft_nms, cpu_mask_soft_nms
...
def mask_soft_nms(dets, sigma=0.6, Nt=0.3, threshold=0.001, method=2):
    keep = cpu_mask_soft_nms(np.ascontiguousarray(dets, dtype=np.float32),
                        np.float32(sigma), np.float32(Nt),
                        np.float32(threshold),
                        np.uint8(method))
    return keep
...
class mask_nms_wrapper(object):
    ...
    def process(self, dets):
        nms_dets = nms(dets, self.thresh) if self.thresh > 0 else mask_soft_nms(dets, sigma=self.sigma, method=2)
        return nms_dets
