import mxnet as mx
import numpy as np
from data_utils.data_workers import im_worker, service_im_worker
...
class MNIteratorTest(MNIteratorBase):
    def __init__(self, roidb, config, test_scale, batch_size=4, threads=8, nGPUs=1, pad_rois_to=400, crop_size=(512, 512),
        ...
        self.im_worker = service_im_worker(crop_size=None if not self.crop_size else self.crop_size[0], cfg=config, target_size=test_scale)
    ...
    def set_scale(self, scale):
        ...
        #                            target_size=scale)
        self.im_worker = service_im_worker(crop_size=None if not self.crop_size else self.crop_size[0], cfg=self.cfg,
                                   target_size=scale)
