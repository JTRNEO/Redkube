...
#include "./multi_proposal_target_mask_satellite-inl.h"
#include <set>
#include <math.h>
#include <unistd.h>
...
__global__ void NonMaximumSuppressionSatelliteCu(float* idets, int post_nms_top_n, int num_images,
  ...
  __shared__ int keeps[1000]; //hard coded, sorry
...
__global__ void getPropsSatelliteCu(float* boxes,
                             float* deltas,
                             float* im_info,
                             float* anchorbuf,
                             float* scores,
...
class MultiProposalTargetMaskSatelliteGPUOp : public Operator{
 public:
  float *labels;
  float *bbox_targets;
  float *bbox_weights;
  ...
  explicit MultiProposalTargetMaskSatelliteGPUOp(MultiProposalTargetMaskSatelliteParam param) {
    this->param_ = param;
    this->param_.workspace = (param_.workspace << 20) / sizeof(float);
    // TODO: change the hard code
    this->crois = new float[1000*param.batch_size*5];
    this->labels = new float[1000*param.batch_size];
    this->gt_boxes = new float[param.max_gts*param.batch_size*5];
    this->bbox_targets = new float[1000*param.batch_size*4];
    this->bbox_weights = new float[1000*param.batch_size*4];
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    utils::getPropsSatelliteCu<<<numblocks, threadsPerBlock>>>(propbuf, tbbox_deltas.dptr_, tim_info.dptr_, anchorbuf, tscores.dptr_,
    ...
    utils::NonMaximumSuppressionSatelliteCu<<<num_images, threadsPerBlock>>>(propbuf, rpn_post_nms_top_n, num_images,
    ...
    if(error != cudaSuccess)
  {
    ...
    for (int i = 0; i < num_images*1000; i++) {
    ...
    // TODO: change hard code
    cudaMemcpy(olabels.dptr_, labels, sizeof(float) * num_images*1000, cudaMemcpyHostToDevice);
    cudaMemcpy(obbox_targets.dptr_, bbox_targets, 4*sizeof(float) * num_images*1000, cudaMemcpyHostToDevice);
    cudaMemcpy(obbox_weights.dptr_, bbox_weights, 4*sizeof(float) * num_images*1000, cudaMemcpyHostToDevice);
  }
...
template<>
Operator *CreateOp<gpu>(MultiProposalTargetMaskSatelliteParam param) {
  return new MultiProposalTargetMaskSatelliteGPUOp<gpu>(param);
}
