...
def visualize_masks(im, detections, seg_masks, scale, pixel_means, class_names, threshold=0.5, save_path='debug.png',
                   transform=True, dpi=80):
    ...
    for j, name in enumerate(class_names):
        ...
        for i in range(len(dets)):
            ...
            ax.add_patch(rect)
            ax.text(bbox[0], bbox[1] - 2 if bbox[1]-2 > 15 else bbox[1]+15, '{:s} {:.1f}'.format(name, score),
                    bbox=dict(facecolor=color, alpha=0.3), fontsize=6, color='white')
            ...
            mask[mask <= 0.5] = 0
            try:
                mask_image[bbox[1]: bbox[3], bbox[0]: bbox[2]] = mask_image[bbox[1]: bbox[3], bbox[0]: bbox[2]] + mask
            except:
                pdb.set_trace()
